...
const PRICE_IDS: Record<string, Record<string, string>> = {
  compleet: {
    month: Deno.env.get('STRIPE_PRICE_COMPLEET_MONTH') ?? '',
    year:  Deno.env.get('STRIPE_PRICE_COMPLEET_YEAR')  ?? '',
  },
  premium: {
    month: Deno.env.get('STRIPE_PRICE_PREMIUM_MONTH') ?? '',
    year:  Deno.env.get('STRIPE_PRICE_PREMIUM_YEAR')  ?? '',
  },
};
